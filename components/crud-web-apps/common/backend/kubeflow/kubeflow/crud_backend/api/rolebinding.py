...
def create_rolebinding(namespace: str, name: str, owner_references=None):
    ...
    try:
        log.info(f'Create: rolebinding {name}')
...
def get_rolebinding(namespace: str, name: str) -> client.V1RoleBinding:
    ...
    log.info(f'Get: rolebinding {name}')
...
def patch_rolebinding(namespace: str, name: str, rb: client.V1RoleBinding):
    try:
        log.info(f'Patch: rolebinding {name}')
...
def delete_rolebinding(namespace: str, name: str):
    try:
        log.info(f'Delete: rolebinding {name}')
